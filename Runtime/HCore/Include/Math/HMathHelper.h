...
#endif
#ifndef H_PIf
#define H_PIf              	(3.1415926535897932384626433832795f)
#endif

// legacy - these should go out and be replaced by std equivalents
#ifndef EPSf
#define EPSf             	    ( std::numeric_limits<float>::epsilon() * 2.0f )
#define EPSd             	    ( std::numeric_limits<double>::epsilon() * 2.0f )
#endif

#ifndef FLOAT_HIGHEST
#define FLOAT_HIGHEST    	    ( std::numeric_limits<float>::max() )
#endif // !FLOAT_HIGHEST
#ifndef FLOAT_LOWEST
#define FLOAT_LOWEST     	    ( std::numeric_limits<float>::lowest() )
#endif
#ifndef FLOAT_MIN_POSITIVE
#define FLOAT_MIN_POSITIVE 	    ( std::numeric_limits<float>::min() )
#endif
#ifndef FLOAT_ONE_MINUS_EPSILON
#define FLOAT_ONE_MINUS_EPSILON  (0x1.fffffep-1)                                         // biggest float smaller than 1 (see OneMinusEpsilon in pbrt book!)
#endif

#ifndef DOUBLE_HIGHEST
#define DOUBLE_HIGHEST   	    ( std::numeric_limits<double>::max() )
#endif
#ifndef DOUBLE_LOWEST
#define DOUBLE_LOWEST    	    ( std::numeric_limits<double>::lowest() )
#endif
#ifndef DOUBLE_MIN_POSITIVE
#define DOUBLE_MIN_POSITIVE 	    ( std::numeric_limits<double>::min() )
#endif
#ifndef DOUBLE_ONE_MINUS_EPSILON
#define DOUBLE_ONE_MINUS_EPSILON (0x1.fffffffffffffp-1)                                  // biggest double smaller than 1 (see OneMinusEpsilon in pbrt book!)
#endif
...
// TODO: also see https://www.pbr-book.org/3ed-2018/Utilities/Mathematical_Routines
	...
	public:
		...
		static inline float         DegreeToRadian(float degree) { return degree * H_PIf / 180.0f; }
		static inline float         RadianToDegree(float radian) { return radian * 180.0f / H_PIf; }
		...
		static inline bool          NearEqual(float a, float b, float epsilon = EPSf) { return HMathHelper::Abs(a - b) < epsilon; }
	...
	struct vaEquidistantSampleLinearGraph
	{
		...
		{
			float   posFlt = HMathHelper::Clamp(pos * (cNumberOfElements - 1), 0.0f, (float)cNumberOfElements - 1.0f);
			int     posIndex = HMathHelper::Clamp((int)posFlt, 0, cNumberOfElements - 2);
			float   posFrac = posFlt - (float)posIndex;

			return HMathHelper::Lerp(Elements[posIndex], Elements[posIndex + 1], posFrac);
		}
	};
	...
	inline float HMathHelper::TimeIndependentLerpF(float deltaTime, float lerpRate)
	{
		return 1.0f - expf(-fabsf(deltaTime * lerpRate));
	}

	template<class T>
	inline T HMathHelper::Min(T const& a, T const& b)
	{
		return (a < b) ? (a) : (b);
	}

	template<class T>
	inline T HMathHelper::Min(T const& a, T const& b, T const& c)
	{
		return Min(Min(a, b), c);
	}

	template<class T>
	inline T HMathHelper::Min(T const& a, T const& b, T const& c, T const& d)
	{
		return Min(Min(a, b), Min(c, d));
	}

	template<class T>
	inline T HMathHelper::Max(T const& a, T const& b)
	{
		return (a > b) ? (a) : (b);
	}

	template<class T>
	inline T HMathHelper::Max(T const& a, T const& b, T const& c)
	{
		return Max(Max(a, b), c);
	}

	template<class T>
	inline T HMathHelper::Max(T const& a, T const& b, T const& c, T const& d)
	{
		return Max(Max(a, b), Max(c, d));
	}

	template<class T>
	inline T HMathHelper::Clamp(T const& v, T const& min, T const& max)
	{
		assert(max >= min);
		if (v < min) return min;
		if (v > max) return max;
	...
	template<>
	inline float HMathHelper::Saturate(float const& a)
	{
		assert(!std::isnan(a));
	...
	// short for clamp( a, 0, 1 )
	template<class T>
	inline T HMathHelper::Saturate(T const& a)
	{
		return Clamp(a, (T)0.0, (T)1.0);
	}

	template<class T>
	inline T HMathHelper::Lerp(T const& a, T const& b, const float f)
	{
		return a + (b - a) * f;
	}
	...
	// why not just use std::swap?
	//template<class T>
	//inline void HMathHelper::Swap(T & a, T & b)
	//{
	...
	template<class T>
	inline T HMathHelper::Sq(const T& a)
	{
		return a * a;
	}

	inline double HMathHelper::Frac(double a)
	{
		return fmod(a, 1.0);
	}

	inline float HMathHelper::Frac(float a)
	{
		return fmodf(a, 1.0);
	}
	...
	inline float HMathHelper::WrapMax(float x, float max)
	{
		return fmodf(max + fmodf(x, max), max);
	}
	//
	inline float HMathHelper::WrapMinMax(float x, float min, float max)
	{
	...
	inline float HMathHelper::AngleWrap(float angle)
	{
		return (angle > 0) ? (fmodf(angle + H_PI, H_PI * 2.0f) - H_PI) : (fmodf(angle - H_PI, H_PI * 2.0f) + H_PI);
	}

	inline float HMathHelper::AngleSmallestDiff(float a, float b)
	{
	...
	inline bool HMathHelper::IsPowOf2(int32 val)
	{
		if (val < 1) return false;
		return (val & (val - 1)) == 0;
	}

	inline bool HMathHelper::IsPowOf2(uint32 val)
	{
		return (val & (val - 1)) == 0;
	}

	inline int HMathHelper::PowOf2Ceil(int val)
	{
		int l2 = Log2(Max(0, val - 1)) + 1;
		return 1 << l2;
	}

	inline int HMathHelper::Log2(int val)
	{
		unsigned r = 0;
	...
	inline float HMathHelper::Log2(float val)
	{
		return log2f(val);
	}

	inline int HMathHelper::Abs(int a)
	{
		return abs(a);
	}

	inline float HMathHelper::Abs(float a)
	{
		return fabsf(a);
	}

	inline double HMathHelper::Abs(double a)
	{
		return fabs(a);
	}

	template<class T>
	inline T HMathHelper::Sign(const T& a)
	{
		if (a > 0) return 1;
		if (a < 0) return -1;
		return 0;
	...
	inline float HMathHelper::Smoothstep(const float t)
	{
		return t * t * (3 - 2 * t);
	}

	inline float HMathHelper::SampleVariance(float samples[], int n)
	{
	...
	inline uint HMathHelper::Hash32(uint x)
	{
	...
	inline float HMathHelper::Hash32NextFloatAndAdvance(uint& hash)
	{
	...
	inline uint32 HMathHelper::Hash32Combine(uint32 seed, const uint32 value)
	{
