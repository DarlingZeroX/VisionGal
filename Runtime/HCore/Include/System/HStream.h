...
namespace Horizon
{
	...
	public:
		...
		inline bool             WriteTXT(const char* fmtStr, ...) { va_list args; va_start(args, fmtStr); string str = HStringTools::Format(fmtStr, args); va_end(args); return WriteTXT(str); }
	};
	...
	template<typename T>
	inline bool HStream::WriteValue(const T& val)
	{
		assert(CanWrite());
	...
	template<typename T>
	inline bool HStream::ReadValue(T& val)
	{
		assert(CanRead());
		return Read(&val, sizeof(T));
	}
	template<typename T>
	inline bool HStream::ReadValue(T& val, const T& deft)
	{
		assert(CanRead());
		if (ReadValue(val))
			return true;
	...
	inline bool HStream::WriteString(const wstring& str)
	{
	...
	inline bool HStream::WriteString(const string& str)
	{
	...
	inline bool HStream::ReadString(wstring& outStr)
	{
	...
	inline bool HStream::ReadString(string& outStr)
	{
		uint32 lengthInBytes;
		if (!ReadValue<uint32>(lengthInBytes))
			return false;
	...
	inline bool HStream::ReadTXT(wstring& outStr, int64 count)
	{
		int64 remainingSize = GetLength() - GetPosition();
		if (count == -1)
			count = remainingSize;
		else
			count = glm::min(count, remainingSize);
	...
	inline bool HStream::ReadTXT(string& outStr, int64 count)
	{
		assert(CanRead());
		int64 remainingSize = GetLength() - GetPosition();
		if (count == -1)
	...
	inline bool HStream::WriteTXT(const wstring& str)
	{
		assert(CanWrite());
		uint32 lengthInBytes = (uint32)str.size() * 2;
		assert(lengthInBytes < (1 << 31));
	...
	inline bool HStream::WriteTXT(const string& str)
	{
		assert(CanWrite());
		uint32 lengthInBytes = (uint32)str.size();
		assert(lengthInBytes < (1 << 31));
...
#define VASTREAM_ALLOW_WHOLE_VECTOR_BUFFER_READWRITE
	...
	template<typename ElementType>
	inline bool HStream::WriteValueVector(const std::vector<ElementType>& elements)
	{
		...
		bool ret = WriteValue<size_t>(elements.size());
	...
	template<typename ElementType>
	inline bool HStream::ReadValueVector(std::vector<ElementType>& elements)
	{
		...
		const int64 sizeToRead = count * sizeof(ElementType);
