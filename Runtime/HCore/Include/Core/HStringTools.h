...
namespace Horizon
{
	...
	class CORE_MODULE_API HMappedString
	{
		const char* m_string = nullptr;

	private:
		friend class HStringDictionary;
		HMappedString(const char* str) : m_string(str) { }
	public:
		HMappedString() { }

		//hMappedString & operator = ( const hMappedString & str ) { m_string }

	public:
		operator const char* () const { return m_string; }
	};

	class HStringDictionary
	{
	...
	private:
		...
		HMappedString MapInternal(StringType str)
		{
	...
	public:
		HMappedString Map(const string& str) { return MapInternal(str); }
		HMappedString Map(const char* str) { return MapInternal(str); }

		// !! WARNING !! any instance of hMappedString is now dangling, so use with great caution (or don't use)
	...
	class CORE_MODULE_API HStringTools
	{
	public:
		...
		static wstring              ToLowerCopy(const wstring& str);
		static string               ToLowerCopy(const string& str);

		static wstring              ToUpperCopy(const wstring& str);
		...
		static shared_ptr<HMemoryStream>
