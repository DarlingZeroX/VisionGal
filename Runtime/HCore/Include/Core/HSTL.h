...
namespace Horizon
{
	...
	class HRecursiveMutex : private std::recursive_mutex
	{
#ifdef _DEBUG
		int m_lockDepth;
#endif
	public:
#ifdef _DEBUG
		HRecursiveMutex() : m_lockDepth(0) { }
		~HRecursiveMutex() { this->lock(); assert(m_lockDepth == 1); this->unlock(); }
		...
		HRecursiveMutex(const HRecursiveMutex&) = delete;
		HRecursiveMutex& operator=(const HRecursiveMutex&) = delete;
	};

	// this needs to go out
	using HRecursiveMutexScopeLock = std::lock_guard<HRecursiveMutex>;

#define VA_RECURSIVE_MUTEX_SCOPE_LOCK( name )             HRecursiveMutexScopeLock VA_COMBINE(localscope,__LINE__)( name )
