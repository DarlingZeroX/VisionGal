...
namespace Horizon
{
    ...
    // 
    // - HTT_Trackee can only be created with a reference to HTT_Tracker, and is then tracked in an array by the tracker.
    // - HTT_Trackee object can only be tracked by one vaTT_Tracker object.
    ...
    private:
        template< class TTTagType >
        friend class HTT_Trackee;
        std::vector< HTT_Trackee<TTTagType>* >            m_tracker_objects;
    ...
    public:
        HTT_Tracker() : m_tracker_objects_lock_already_locked(false) { }
        virtual ~HTT_Tracker();
    ...
    class HTT_Trackee
    {
        typedef HTT_Tracker<TTTagType>     HTT_TrackerT;
        friend HTT_TrackerT;
    private:
        HTT_TrackerT* m_tracker;
        int                                 m_index;
        TTTagType const                     m_tag;

    public:
        HTT_Trackee(HTT_TrackerT* tracker, TTTagType tag)
            : m_tag(tag)
        {
            m_tracker = tracker;
    ...
    public:
        const HTT_TrackerT* TT_GetTracker() const { return m_tracker; };
    ...
    template< class TTTagType >
    inline HTT_Tracker<TTTagType>::~HTT_Tracker()
    {
    ...
    class HHashedCircularCache
    {
        std::array< HCircularCache<_Key, _Element, _Size>, _Buckets >
            m_buckets;
        _Hasher                         m_hasher;
    ...
    public:
        HSparseArray() { Validate(); }
        ~HSparseArray() { Validate(); Clear(); }
